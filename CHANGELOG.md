...
- **`tt/tt/full_ast_emit.py`** — full AST pipeline (replaces `roai_hybrid_emit`): copy runtime module, build thin `RoaiPortfolioCalculator` facade with `ast`, append TS-derived `_body_*` methods. Enable with `emit_full_ast: true` in `tt_project_config.py`.
...
- **`tt/tt/translator.py`** — Loads **`tt_project_config.py`** via `load_project_config_module()` when present; falls back to legacy **`tt_import_map.json`**. Calls **`emit_from_spec()`** with resolved config directory. **`try_emit_full_ast()`** runs first when `emit_full_ast` is set in `CONFIG` (legacy `emit_roai_hybrid` still accepted).
